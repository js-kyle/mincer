...
var DIRECTIVE_HANDLERS = {
  ...
  // `require_glob` requires all files matching a glob pattern.
  //
  //     //= require_glob "./components/**/scripts/*.js"
  //
  require_glob: function (self,args) {
    var root, pathglob = isRelative(args[0]) ? args[0] : ('./' + args[0]);
    root = path.dirname(self.pathname)
    var files = glob.sync(pathglob,{cwd: root});
