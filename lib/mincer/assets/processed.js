...
/**
 *  ProcessedAsset#isFresh() -> Boolean
 *
 *  Checks if Asset is stale by comparing the actual mtime and
 *  digest to the inmemory model.
 **/
ProcessedAsset.prototype.isFresh = function (env) {
  return _.all(this.__dependencyPaths__, function (dep) {
    return Asset.isDependencyFresh(env, dep);
  });
};
...
getter(ProcessedAsset.prototype, 'buffer', function () {
  if (!this.__buffer__) {
    throw new Error("Can't read buffer. Asset wasn't compiled yet.");
  }
