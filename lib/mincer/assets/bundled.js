...
var BundledAsset = module.exports = function BundledAsset() {
  var processedAsset, Klass, context, processors, options,
      source = '', resultSourceNode;//, sourceMapOffset;
  ...
  if (!this.environment.isEnabled('source_maps')) {
    ...
    resultSourceNode = new sourcemap.SourceNode(0, 0, this.relativePath);

    this.toArray().forEach(function (dep) {

      var depContent = dep.toString();
      var depName = dep.relativePath;
      var depSourceNone;

      if (dep.sourceMap) {
        // Sourcemap exists - load it
        depSourceNone = sourcemap.SourceNode.fromStringWithSourceMap(
                              depContent,
                              new sourcemap.SourceMapConsumer(dep.sourceMap)
                            );
      } else {
        // Sourcemap not exists - create it & load
        var smg = new sourcemap.SourceMapGenerator({ file: depName });
        ...
        smg.setSourceContent(depName, depContent);

        depSourceNone = sourcemap.SourceNode.fromStringWithSourceMap(
                              depContent,
                              new sourcemap.SourceMapConsumer(smg.toString())
                            );
        // TODO: why file name not added?
        /*depSourceNone = new sourcemap.SourceNode(
                              1,
                              0,
                              depName,
                              depContent.split('\n'),
        ...
        depMap.setSourceContent(dep.relativePath, dep.toString());*/
      }

      resultSourceNode.add(depSourceNone);
    });

    //console.log('######## bundled asset: ', resultMap.toString());
    var sm = resultSourceNode.toStringWithSourceMap();
    this.sourceMap = sm.map.toString();
    source = sm.code;
  }
