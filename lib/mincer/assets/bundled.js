...
var BundledAsset = module.exports = function BundledAsset() {
  ...
  this.__buffer__ = new Buffer(context.evaluate(this.pathname, options));
...
BundledAsset.prototype.initWith = function (environment, hash) {
  ...
  this.__buffer__ = new Buffer(hash['source']);
  ...
  if (this.__processedAsset__.dependencyDigest !== hash['requiredAssetsDigest']) {
    // AFAIK should never happen in real life
    throw "processed asset belongs to a stale environment";
  }
};
