...
var BundledAsset = module.exports = function BundledAsset() {
  ...
  if (!this.environment.isEnabled('source_maps')) {
    ...
    this.toArray().forEach(function (dep) {
      var map = JSON.parse(dep.sourceMap);
      var relDir = path.relative(baseDir, path.dirname(dep.pathname));

      // Convert dependency paths to be relative to current dir
      map.sources.forEach(function (file, idx) {
        var rel = path.normalize(path.join(relDir, file));
    ...
    var result = resultSourceNode.toStringWithSourceMap();
    var sm = JSON.parse(result.map.toString());
    // Set back source root
    sm.sourceRoot = path.dirname(this.relativePath);
    if (path.sep === '\\') { sm.sourceRoot = sm.sourceRoot.replace('\\', '/'); }

    this.sourceMap = JSON.stringify(sm);
    source = result.code;
  }
