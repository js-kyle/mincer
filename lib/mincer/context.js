...
function beautifyError(err, pathname, klass) {
  var b = new Error(String(klass.name || "<anonymous processor>") + ": " +
...
Context.prototype.evaluate = function (pathname, options, callback) {
  var self = this, attributes, processors, error = null, result, locals = {};
  ...
  _.forEach(processors, function (ProcessorKlass) {
    var template;

    try {
      template = new ProcessorKlass(pathname, result);
    } catch (err) {
      error = beautifyError(err, pathname, ProcessorKlass);
      return false;
    }
    ...
    try {
      ...
      error = beautifyError(err, self.pathname, ProcessorKlass);
      return false;
    }
  });
