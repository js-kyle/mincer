...
Context.prototype.evaluate = function (pathname, options, callback) {
  var self = this, attributes, processors, result, locals = {}, extra;
  ...
  async.forEachSeries(processors, function (ProcessorKlass, next) {
    ...
    template.evaluate(self, locals, function (err, data) {
      ...
      // If when compiling the template, it produced a source map we
      // pass it through to the Asset.
      if (template.sourceMap) {
        extra = {};
        extra.sourceMap = template.sourceMap;
        extra.originalSource = template.data;
      }
