...
Context.prototype.evaluate = function (pathname, options, callback) {
  ...
  async.forEachSeries(processors, function (ProcessorKlass, next) {
    ...
    // Some templates can be used to generate pre-compiled template functions with JST.
    // This templates can use functions instead of strings as inputs to render...
    var source = template.data || '';
    if (source instanceof Function && !template.acceptCallable) {
      if (source.solve && source.solve instanceof Function) {
        template.data = source.solve();
      } else {
        template.data = source.call(_.extend(_.clone(self), locals));
      }
    }
