...
Context.prototype.resolve = function (pathname, options, fn) {
  var self = this, attributes, content_type;
  ...
  if ('/' === pathname[0]) {
    ...
    return this.resolve(pathname, {}, function (candidate) {
...
Context.prototype.requireAsset = function (pathname) {
  ...
  if (!pathname) {
    // FIXME: resolve returns undeinfed sometimes...
    console.warn("Oh no it happened again - empty path...");
    return;
  }
...
Context.prototype.evaluate = function (pathname, options, callback) {
  var self = this, attributes, processors, result;
  ...
  async.forEachSeries(processors, function (ProcessorKlass, next) {
    ...
    template.evaluate(self, {}, function (err, data) {
      result = data;
      next(err);
    });
  }, function (err) {
