...
Context.prototype.evaluate = function (pathname, options) {
  ...
  var smg, filePath, sourceRoot; // for sourcemap
  ...
  // Generate dummy (1:1) source map, if not exists & option enabled.
  // That simplifies checks in followed calls.
  if (this.environment.isEnabled('source_maps') && !this.sourceMap) {

    filePath = path.basename(this.pathname); // relative to self
    sourceRoot = path.dirname(this.relativePath);
    if (path.sep === '\\') { sourceRoot = sourceRoot.replace('\\', '/'); }
    ...
    smg.setSourceContent(filePath, result.toString());

    this.sourceMap = smg.toString();
  }
