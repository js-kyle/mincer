...
/**
 *  new Template(file, line, options[, reader])
 *  - file (String)
 *  - line (Number)
 *  - options (Object)
 *  - reader (Function)
 ...
 *  After all calls [[Template#prepare]].
 **/
var Template = module.exports = function Template(file, line, options, reader) {
 ...
 *  - [[Template#requireTemplateLibrary]]
 **/
Template.prototype.initializeEngine = function () {};


/**
 *  Template#prepare() -> Void
 *
 *  Runs once instance of template is created and engin is initialized.
 *
 *  You _MAY_ redefine this method in your template if you need some preparation
 *  logic. Default implementation does nothing.
 *
 *
 *  ##### Example
 *
 *      MyProcessor.prototype.prepare = function () {
 *        this.secret = (backend.SECRET_RE.exec(this.data) || [])[1];
 *      };
 **/
...
/**
 *  Template#requireTemplateLibrary(name) -> Mixed
 *  - name (String)
 *
 *  Wrapper over native `require()` method, that produces beautified errors.
 *  If your template requires 3rd-party module, use this method inside your
 *  [[Template#initializeEngine]] method instead of simple `require()`.
 **/
Template.prototype.requireTemplateLibrary = function (name) {
  // TODO: catch and "beautify" errors
  return require(name);
};
