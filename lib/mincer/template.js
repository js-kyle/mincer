...
var Template = module.exports = function Template(file, reader) {
  ...
  prop(this, 'file', file);
  prop(this, 'acceptCallable', false, {writable: true});
...
/**
 *  Template#acceptCallable -> Boolean
 *
 *  If your engine accept `[[Template#data data]]` as functions for lazy
 *  evaluation you need to define `acceptCallable` as true, otherwise all
 ...
 *  method as the source to be processed.
 *
 *  ##### Example
 *
 ...
 *      };
 *
 *  The `evaluate` method could pass to the callback function an string as a
 *  result of the asset compilation or a function for lazy evaluation.
 *
 *  Passing a function to the callback is useful when precompiling with template
 *  engines like `eco` or `ejs`. The `JST` processor can use the function source
 *  to generate pre-compiled templates while other processors can use the
 *  returned string to perform future compilations. If the following processors
 *  in the asset pipeline does not support functions for lazy evaluation, you
 *  can provide a method to transform the function in string setting a `solve`
 *  member for the function. If the `solve` member is not available, the
 *  function will simply be called with the next processor context.
 *
 *  ##### Example
 *
 *      OtherProcessor.prototype.evaluate = function (context, locals, callback) {
 *        var fn = new Function('return ' + this.data.replace("{{var}}", this.variable));
 *        fn.solve = function() { return fn.call(context, locals); }
 *        callback(null, fn);
 *      };
 *
 *  ##### See Also
