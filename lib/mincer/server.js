...
var Server = function Server(environment, manifest) {
...
function httpdate(mtime) {
  ...
  try {
    date = new Date(Date.parse(mtime));
  } catch (err) {
    logger.error("Failed parse mtime '" + mtime + "': " +
...
Server.prototype.onRequest = function (req, res) {
  ...
  asset.compile(function (err) {
    ...
    res.setHeader("Last-Modified",  httpdate(asset.mtime));
