...
// small helper to set compressed buffer cache on asset.
...
Server.prototype.compile = function compile(pathname, bundle, callback) {
  ...
  // got asset without cached buffer - cache it
  if (undefined === asset.__buffer__) {
    prop(asset, '__buffer__', asset.buffer);
  }

  // Cache gzipped buffer if asset is gzipable
  if (is_compressable(asset) && (undefined === asset.gzipped)) {
    set_compressed_cache(zlib.gzip, asset, 'gzipped', function (err) {
      callback(err, asset);
      return;
    });
    return;
