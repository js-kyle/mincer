...
Server.prototype.handle = function (req, res) {
  ...
  // if manifest provided - die faster if pathname not found there,
  // so no FS operations will be taken in any case
  if (this.manifest && !this.manifest.assets[pathname]) {
    logger.info(msg_prefix + "404 Not found within manifest " + timer.stop());
    end(res, 404);
    return;
  }
