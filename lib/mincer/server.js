...
function serve_asset(self, asset, req, res, timer) {
  ...
  buffer = asset.__buffer__;
  length = asset.__buffer__.length;
  ...
  if (asset.sourceMap) {
    ...
    res.setHeader('X-SourceMap', source_map_url); // Deprecated header
    res.setHeader('SourceMap',   source_map_url);
  }
...
function serve_source_map(self, asset, req, res, timer) {
  ...
  if (!asset.__source_map_buffer__) {
    ...
    buffer = asset.__source_map_buffer__;
    length = asset.__source_map_buffer__.length;

    if (asset.__gzipped_source_map__ && is_gzip_accepted(req)) {
      buffer = asset.__gzipped_source_map__;
      length = asset.__gzipped_source_map__.length;
      res.setHeader('Content-Encoding', 'gzip');
    }
...
Server.prototype.compile = function compile(pathname, bundle, callback) {
  ...
  if (asset.sourceMap) {
    prop(asset, '__source_map_buffer__', new Buffer(asset.sourceMap));
  }
  ...
  zlib.gzip(asset.__buffer__, function (err, buffer) {
    ...
    zlib.gzip(asset.__source_map_buffer__, function (err, buffer) {
      if (err) {
        callback(err);
        return;
      }
      ...
      // set __gzipped__ buffer only if we have compression profit
      if (buffer.length < asset.__source_map_buffer__.length) {
        prop(asset, '__gzipped_source_map__', buffer);
      }
