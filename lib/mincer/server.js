...
var GZIPABLE_MIME_RE = /text|json|javascript/;
function is_gzipable(asset) {
  return GZIPABLE_MIME_RE.test(asset.contentType);
}
...
Server.prototype.compile = function (pathname, callback) {
  ...
  asset.compile(function (err/*, asset */) {
    ...
    if (is_gzipable(asset) && undefined === asset.gzipped) {
      zlib.gzip(asset.buffer, function (err, gzipped) {
        if (!err) {
          // assign property only if success
          prop(asset, 'gzipped', gzipped);
        }
...
Server.prototype.handle = function (req, res) {
  ...
  this.compile(pathname, function (err, asset) {
    ...
    //
    // Vary header is important for proxies
    // TODO: Explain WHY Vary is important and what it does
    //
