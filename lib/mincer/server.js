...
Server.prototype.handle = function handle(req, res) {
  ...
  var coffeeRedux = require('/Users/Vaughan/dev/CoffeeScriptRedux');

  // `?source=1` was found in query string.
  // Serve CoffeeScript source.
  var _asset;
  if (source || (path.extname(pathname) === '.coffee')) {
    _asset = self.environment.findAsset(pathname);
    var coffeeSource = fs.readFileSync(_asset.pathname, 'utf8');
    res.setHeader('Content-Type', 'application/javascript');
    return res.end(new Buffer(coffeeSource));
  }

  // Handle `.map` file request.
  if (path.extname(pathname) === '.map') {
    var _pathname = pathname.slice(0, -4);
    _asset = self.environment.findAsset(_pathname);
    if (path.extname(_asset.pathname) === '.coffee') {
      var input = fs.readFileSync(_asset.pathname, 'utf8');
      var coffee = coffeeRedux;
      var csAst = coffee.parse(input);
      var jsAst = coffee.compile(csAst);
      // Use logical path with original extensions.
      // i.e. `<logicalPath>.<sourceFileExtensions>`
      var sourceMap = coffee.sourceMap(jsAst, path.basename(_asset.logicalPath, '.js') + path.extname(_asset.pathname));
  ...
  this.compile(pathname, bundle, function (err, asset) {
    ...
    // Let browser know where to find the source map if this is a CoffeeScript
    // file.
