...
function serve_asset(self, asset, req, res, timer) {
  ...
  if (asset.sourceMap) {
    smap_url = url.parse(req.originalUrl || req.url);
    smap_url.pathname += '.map';
    smap_url = url.format(smap_url);

    res.setHeader('X-SourceMap', smap_url); // Deprecated header
    res.setHeader('SourceMap',   smap_url);
  }
...
function serve_smap(self, asset, req, res, timer) {
  var buffer;
...
Server.prototype.handle = function handle(req, res) {
  ...
  this.compile(pathname, bundle, function (err, asset) {
    ...
    (sourceMap ? serve_smap : serve_asset)(self, asset, req, res, timer);
  });
};
