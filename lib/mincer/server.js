...
Server.prototype.handle = function handle(req, res) {
  var self        = this,
      ...
      source      = /source=[1t]/.test(url.parse(req.url).query),
  ...
  // If the request is for a source map we need to make sure we have compiled
  // the asset first.
  // NOTE: Source mapping disabled for bundled assets for now.
  if (path.extname(pathname) === '.map') {
    var jsPathname = pathname.slice(0, -4);
    var jsAsset = self.environment.findAsset(jsPathname);
    if (path.extname(jsAsset.pathname) === '.coffee') {
      this.compile(jsPathname, false, function (err, asset) {
      ...
      // If the requested source map does not correspond to a Coffeescript file
      // we fail, because only Coffeescript source maps are currently
      // supported.
      end(res, 500);
      return;
    }
  ...
  // If requested file ends with `.coffee` or `?source=1` was found in
  // query string, serve original source of Coffeescript file.
  // The browser will make this request when it finds a `.coffee` file
  // referenced from within a source map.
  if (source || (path.extname(pathname) === '.coffee')) {
    this.compile(pathname, false, function (err, asset) {
      res.setHeader('Content-Type', 'application/javascript');
      res.setHeader('Content-Length', asset.originalSource.length);
      return res.end(new Buffer(asset.originalSource));
    });
    return;
  }
  ...
  this.compile(pathname, bundle, function (err, asset) {
    ...
    //
    // Let browser know where to find the source map if this asset
    // is a CoffeeScript file.
    // TODO: Provide option to disable source maps.
    //
    if (path.extname(asset.pathname) === '.coffee' && !bundle) {
      // Browser looks for source maps relative to it current path which is
      // why we used the basename instead of the logicalPath.
      res.setHeader('X-SourceMap', path.basename(asset.logicalPath) + '.map');
    }
