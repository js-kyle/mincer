...
Base.prototype.resolve = function (logicalPath, options, fn) {
  ...
  if (fn) {
    ...
    return this.__trail__.find(args, options, function (pathname) {
      var component, extname, mainfile;

      if ('component.json' !== path.basename(pathname)) {
        return fn(pathname);
      }

      component = require(pathname);
      extname   = path.extname(pathname);

      if (_.isString(component.main)) {
        return fn(path.join(path.dirname(pathname), component.main));
      }

      if (_.isArray(component.main)) {
        while (component.main.length) {
          mainfile = component.main.shift();
...
// Stores a cache of located paths during precompile.
prop(Base.prototype, 'paths_cache', [], {writable: true});

/**
 *  Base#precompile(files[, options, callback]) -> Void
 *  - files (Array):
 *  - options (Object):
 *    `cachedPaths`: if true, cache paths of assets.
 *  - callback (Function):
 *
 ...
 *  to be synchronous).
 **/
Base.prototype.precompile = function (files, options, callback) {
  var self     = this,
      data     = {files: {}, assets: {}},
      paths    = [];

  // Handle optional 2nd argument.
  ...
  // Set option defaults.
  options.cachePaths = options.cachePaths || false;

  // If `cachedPaths` option is true, cache paths on first precompile.
  // NOTE: `this.eachLogicalPath` is slow because it calls `fs#stat` on each file
  // in each search path tree.
  if (options.cachePaths === false || self.paths_cache.length === 0) {
    this.eachLogicalPath(files, function (pathname) {
      paths.push(pathname);
    });
    self.paths_cache = paths;
  } else {
    paths = self.paths_cache;
  }
