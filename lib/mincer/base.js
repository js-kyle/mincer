 ...
 *
 *  Helper to make sure givel list of `files` were compiled.
 *  Similar to [[Manifest#compile]], but does not write anything to disk.
 *
 *      environment.precompile("app.js", function (err, assets) {
 *        assets[0]; // - compiled asset of 'app.js'
 *      });
 **/
Base.prototype.precompile = function () {
  var self     = this,
      files    = _.flatten(arguments),
      callback = _.isFunction(_.last(files)) ? files.pop() : function () {},
      paths    = [];
  ...
  async.map(paths, function (pathname, next) {
    var asset = self.findAsset(pathname);
