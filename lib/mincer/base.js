...
var StaticAsset     = require('./static_asset');
var ProcessedAsset  = require('./processed_asset');
var BundledAsset    = require('./bundled_asset');
 ...
 *  Base#digest -> Hmac
 *
 *  Returns a `Hmac` instance for the `Environment`.
 *
...
getter(Base.prototype, 'digest', function () {
  ...
  var digest = crypto.createHmac(this.__digestAlgorithm);

  // Compute the initial digest using the implementation class. The
  // Sprockets release version and custom environment version are
  // mixed in. So any new releases will affect all your assets.
  digest.update(VERSION);
  digest.update(this.version);


  Object.defineProperty(digest, 'hexdigest', {
    get: function () {
...
Base.prototype.fileDigest = function (pathname) {
  var stat = this.stat(pathname);

  if (stat && stat.isFile()) {
    // If its a file, digest the contents
    return this.digest.update(fs.readFileSync(pathname));
  } else if (stat && stat.isDirectory()) {
    // If its a directive, digest the list of filenames
    return this.digest.update(this.entries(pathname).join(','));
  }
};
